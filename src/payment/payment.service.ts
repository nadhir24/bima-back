...
export class PaymentService {
  ...
  async cleanupPendingInvoices() {
    try {
      ...
      this.logger.log(
        `Found ${invoiceIds.length} expired PENDING invoices to delete: ${invoiceIds.join(', ')}`,
      );
      ...
      this.logger.log(
        `Deleted ${deletedItems.count} invoice items from expired invoices`,
      );
      ...
      this.logger.log(
        `Successfully deleted ${result.count} expired PENDING invoices`,
      );
      return {
        success: true,
  ...
  async getGuestInvoices(guestId: string, page: number, limit: number) {
    try {
      ...
      this.logger.error(
        `Error fetching invoices for guest ID: ${guestId}`,
        error,
      );
      return {
        success: false,
  ...
  async getAllInvoices(
    ...
    try {
      this.logger.log(
        `Fetching all invoices for admin (page ${page}, limit ${limit}, search: ${search || 'none'}, status: ${status || 'all'}, date range: ${startDate || 'none'} to ${endDate || 'none'})`,
      );
